...
/// <summary>
/// <b>Delegate-backed</b> multi-way branch (the <c>Relay*</c> family — a state whose decision
/// closes over code). Branches like a switch/case. Finishes immediately with
/// <see cref="Result.Success"/>; the runtime asks <see cref="SelectNext"/> for the next node.
/// <para>
/// A closure cannot ride a serialization payload, so a graph branching through this state
/// cannot round-trip and therefore cannot survive suspend/resume. When the tested value is a
/// blackboard slot and the cases are literals, use the data-built
/// <see cref="SwitchState{T}"/> instead.
/// </para>
...
/// </summary>
public sealed class RelaySwitchState<TKey> : ILogic, IDirector, IBlackboardSettable
    where TKey : notnull
{
