...
/// <summary>
/// A synchronous state machine with an agent, mirroring <see cref="StateMachine{TAgent}"/>.
/// </summary>
public class SyncStateMachine<TAgent> : SyncStateMachine, IAgentSettable<TAgent>
{
    public SyncStateMachine(Graph graph, ISyncStateMachineObserver? observer = null)
        : base(graph, observer) { }
...
/// <summary>
/// Synchronous, zero-allocation counterpart of <see cref="StateMachine"/>.
/// <para>
...
///   <item>Observer callbacks are <c>void</c> (see <see cref="ISyncStateMachineObserver"/>).</item>
/// </list>
/// </para>
/// Node logic is executed via <see cref="ISyncLogic.Execute"/> — every node in the graph
/// must have its <see cref="INode.Logic"/> implement <see cref="ISyncLogic"/>.
/// </summary>
public class SyncStateMachine
{
    public readonly Graph Graph;
    private readonly ISyncStateMachineObserver? _observer;
    ...
    private void OnEnter()
    {
        if (_executeGate)
        {
            throw new InvalidOperationException("SyncStateMachine is already executing.");
        }
    ...
    private Result InternalRun()
    {
        while (true)
        {
            ...
            if (logicNode.Logic is SyncState syncStateForLog)
            {
                syncStateForLog.SyncLogReport = _cachedLogReportCallback;
            }

            // Execute the node synchronously.
            if (logicNode.Logic is not ISyncLogic syncLogic)
            {
                throw new InvalidOperationException(
                    $"Node '{_current}' logic ({logicNode.Logic.GetType().Name}) does not implement ISyncLogic. " +
                    "All nodes in a SyncStateMachine must implement ISyncLogic.");
            }
